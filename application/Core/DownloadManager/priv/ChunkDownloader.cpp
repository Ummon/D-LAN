...
void ChunkDownloader::run()
{
   ...
   catch (FM::hashMissmatchException)
   {
      ...
      L_USER(QString(tr("Corrupted data received for the file \"%1\" from peer %2. Peer blocked for %3 ms")).arg(this->chunk->getFullPath()).arg(this->currentDownloadingPeer->getNick()).arg(BLOCK_DURATION));
